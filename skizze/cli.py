...
def ensure_venv() -> None:
    """Ensure that the script runs inside a virtual environment."""
    if os.environ.get("SKIZZE_SKIP_VENV") == "1":
        logger.info("Skipping venv creation due to environment variable.")
        return
    ...
    if str(sys.prefix).startswith(str(venv_dir)):
        logger.debug("Already running inside venv: %s", venv_dir)
        return

    if not venv_dir.exists():
        logger.info("🔧 Erstelle virtuelle Umgebung in './env' …")
...
def install_missing_packages(packages) -> None:
    """Install packages that are missing."""
    if os.environ.get("SKIZZE_SKIP_INSTALL") == "1":
        logger.info("Skipping package installation due to environment variable.")
        return
    for pkg in packages:
        try:
            __import__(pkg)
        except ImportError:
            logger.info("📦 Paket '%s' fehlt. Installiere es jetzt …", pkg)
...
def process_image(
    ...
    logger.info("Verarbeite Bild %s", path)
    ...
    if output_dir:
        ...
        logger.info("Speichere Ergebnisse nach %s", output_dir)
...
def process_batch(paths, output_dir, args):
    ...
    logger.info("Starte Batch-Verarbeitung von %d Bildern", len(paths))
